...
for (n in years) {
  ...
  start <- "stop" #start variable is set "no" for condition to work, no start possible at beginning of a new deployment
  end <- "stop"
  ...
  for (d in 1:deployment){
    ...
    if(nrow(df_ppp_temp) == 0){next} #e.g. 2011 deployment 1 is empty, would stop there otherwise
    ...
    } else {a <- a} #while we are in a ppp (checked by start == start) the control number stays the same, if start was no, it is counted up, for every loop it is "no" variable gets counted up, because of that, reassignment at the end still necessary, but reliable, becuase not with date gap, but with ppp gap
  
      if (start == "stop" & end == "stop" | start== "go" & end == "stop"){
    ##start criteria
...
#   if(is.na(df_ppp_temp$ppp[x:roll_index_df] == TRUE)){ #:roll_index_df
#    df_ppp_temp$ppp[x:roll_index_df] = as.numeric(a) #loop date x : minimum CT within 7 days forward get #assigned the same ppp
#    
#   } else{is.na(df_ppp_temp[x:roll_index_df])}

    replace <- as.numeric(c(df_ppp_temp$ppp[x:roll_index_df]))
    ifelse(is.na(df_ppp_temp$ppp[x:roll_index_df]),
    df_ppp_temp$ppp[x:roll_index_df] <- as.numeric(a),
    df_ppp_temp$ppp[x:roll_index_df] <- replace)    
    
    
    } else{#df_ppp_temp$ppp[x] = NA
           start <- "stop"}
    rm(roll_index_df)
    } else {}#start <- "stop"} #if we don't have a roll_value or the criterion of decrease is not met start is set FALSE, so that end criteria don't kick in without a start
      } else {}
    ##end criteria
    
    #was there a start? then same procdure as for start criterion but looking backwards
    if (start == "go" | start == "stop" & end == "go"){
 ...
 rm(roll_index_df)
    } else{end <- "stop"} #if criterion is not met, we do nothing as NAs are already assigned in start and this would potentially overwrite an existing ppp entry
    
    } else {} #if we can't look backwards, we jump to the next loop iteration
    
    rm(lag_end)
    
    }else {end <- "stop"
      start < "stop"}
      #a <- a+1 }#end start == TRUE condition
...
fm_ppp_final <- fm_ppp_na %>% 
  group_by(year) %>%
  #mutate(contvar = cumsum(c(TRUE, abs(diff(date)>=7)))) %>%
  #mutate(ppp_2 = contvar*ppp) %>% 
