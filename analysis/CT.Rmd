...
df_NA$deployment_2 <- factor(df$deployment,levels=levels(df$deployment))
  
df %>% 
  ggplot(aes( x = date, y = year))+
  geom_point(aes(color = deployment_2))

# second try, no sufficent result
#df %>% 
#  ggplot (aes( x = year, y = month))+
#    geom_col(aes(fill = deployment_2))+
#    coord_flip()+
