...
```{r primary_production_periods, eval=FALSE}
...
#ppp identified per year, per deployment
...
df_ppp_temp <- df_ppp %>% 
  ...
  mutate(ppp = NA)
    ...
    lag <- df_ppp_temp %>% 
    filter(date >= date[x], date <= (date[x]+ duration(timespan, 'days')))

    roll <- which.max(lag$date) 
    roll <- lag$CT[roll]
  
  
    
  if (df_ppp_temp$CT[x]-roll >= 20){
    
    df_ppp_temp$ppp[x] = as.numeric(x)
    #df_ppp_temp <- df_ppp_temp %>% 
    #  mutate(ppp[x] = x)
  } else{
    
    df_ppp_temp$ppp[x] = NA
    #df_ppp_temp <- df_ppp_temp %>% 
    #  mutate(df_ppp_temp$ppp[x] = NA)
  }
  print(paste("year:", n,
              " *** deployment:", d,
              " *** day:", x,
