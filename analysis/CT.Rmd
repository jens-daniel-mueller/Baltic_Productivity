...
for (n in years){
  ...
  for (d in 1:deployment){
    ...
    start <- TRUE #condition for end-criteria; only TRUE when there was the necessary decrease
    
    df_ppp_temp$ppp[x:roll_index_df] = as.numeric(x) #loop date x : minimum CT within 7 days forward get assigned the same ppp
 
    } else{df_ppp_temp$ppp[x] = NA
           start <- FALSE}
    rm(roll_index_df)
    } else {start <- FALSE} #if we don't have a roll_value or the criterion of decrease is not met start is set FALSE, so that end criteria don't kick in without a start
    ...
    roll_index <- which.min(lag_end$CT)
    ...
    df_ppp_temp$ppp[x:roll_index_df] = as.numeric(x) #loop date x : minimum CT within 7 days backwards get assigned the same ppp, if criterion was met
 rm(roll_index_df)
    ...
    rm(start, roll_index, roll_value, lag_start)
...
fm_ppp_final <- fm_ppp_dup_na %>% 
 mutate(ppp = as.factor(cumsum(c(TRUE,diff(date)>= deployment_gap))))
