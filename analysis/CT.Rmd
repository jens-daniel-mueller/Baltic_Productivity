...
fm_ngs_all_routes %>%  
  write.csv(here::here("data/_summarized_data_files/", file = "fm_ngs_all_routes.csv"))
...
gt_windspeed_ngs %>% 
  write.csv(here::here("data/_summarized_data_files/", file = paste("gt_windspeed_ngs.csv")))
...
df <- read.csv(here::here("data/_summarized_data_files/", file =  "fm_ngs_all_routes.csv"), as.is = TRUE)
...
df %>% 
   write.csv(here::here("data/_summarized_data_files/", file = "fm_CT_ngs.csv"))
...
df_1 <- read.csv(here::here("data/_summarized_data_files/", file = "fm_CT_ngs.csv"), as.is = TRUE)
df_2 <- read.csv(here::here("data/_summarized_data_files/", file = "gt_windspeed_ngs.csv"), as.is = TRUE)

df_2$date_time <- round_date(as.Date(df_2$date_time, unit = "day"))
df_2 <- df_2 %>% 
...
df %>% 
   write.csv(here::here("data/_merged_data_files/", file = paste("gt_fm_flux_ngs.csv")))
...
gt_fm_flux_ngs <- read.csv(here::here("data/_merged_data_files/", file = "gt_fm_flux_ngs.csv"), as.is = TRUE)

gt_fm_flux_ngs$date <- as.Date(gt_fm_flux_ngs$date)
...
# read MLD data
gt_mld_fm_pco2_ngs <-
read.csv(here::here("data/_merged_data_files/", file = "gt_mld_fm_pco2_ngs.csv"))
...
df <- df %>% 
  ...
  mutate (month = month(date),
          year = year(date)) %>% 
  filter (month >= time_low & month <= time_high) %>% 
  mutate(deployment = (cumsum(c(TRUE,diff(date)>=7)))) # deployment value +1, when a data gap bigger than 7 days is found
...
df_NA %>% 
  ...
  geom_point(aes(color = deployment_2))#+
  #scale_color_brewer(getPalette(colourCount))

rm(df_Na, df, generic_date)
