...
df %>% 
   vroom_write(here::here("data/_merged_data_files/", file = paste("fm_CT_ngs.csv", sep = "")))
...
```{r calculate_gas_exchange_from_Finnmaid_pCO2_S_T_and_GETM_wind}

df_1 <- vroom::vroom(here::here("data/_merged_data_files/", file = "fm_gt__sal_tem_pco2_ngs.csv"))
...
fm_CT_ngs <- vroom::vroom(here::here("data/_merged_data_files/", file = "fm_CT_ngs.csv"))
gt_windspeed_ngs <- vroom::vroom(here::here("data/_merged_data_files/", file = "gt_windspeed_ngs.csv"))


ts_xts_flux <- xts(gt_fm_flux_ngs$flux_daily, order.by = gt_fm_flux_ngs$date)
names(ts_xts_flux) <- "Daily Flux"
ts_xts_CT <- xts(fm_CT_ngs, order.by = fm_CT_ngs$date)
names(ts_xts_CT) <- "CT"
