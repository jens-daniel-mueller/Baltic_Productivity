...
timeseries$date_time <- as.POSIXct(timeseries$date_time) %>% 
...
# variables: date_time, var, lat, value_Finn, value_GETM, sd_Finn, min_Finn, max_Finn
# use full_join/semi_join/merge (?) function on dates, var to combine



comparison <- full_join(timeseries, SST_GETM_mean, by = c("date_time", "var"))
...
```{r timeseries_plot, eval=FALSE, include=FALSE}
...
ts <- full_join(SST_GETM_mean, timeseries_mean)

ts <- ts %>% 
  select(date_time, Finnmaid = value_mean, GETM=value)
