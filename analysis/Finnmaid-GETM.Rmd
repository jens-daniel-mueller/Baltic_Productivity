...
The previous steps have been performed on only a small coordinate space. Now we want to analyse the differences in SST readings through time and in relation to the distance to Travemuende [km]. 
...
SST_combined_GETM_Finn <-inner_join(SST_Finn, SST_GETM, by = c("date", "corvector"))
...
comparison <- SST_combined_GETM_Finn_dropNA

comparison$week <- as.Date(cut(comparison$date, breaks="weeks"))
comparison$dist.trav <- comparison$corvector*2
comparison$difference_abs <- abs(comparison$value_SST_Finn-comparison$value_SST_GETM)
...
comparison_hov_data <- comparison %>%
      select(value_SST_Finn, value_SST_GETM, difference, difference_abs,week, dist.trav) %>% 
...
hov1 <-
      ...
      scale_fill_viridis_c(name="mean difference in SST [°C]", direction = -1)+
...
hov1    
#Hovmoeller Plot,absolute difference values SST
hov2 <-
    
       comparison_hov_data %>% 
      filter(!is.na(difference_abs_mean)) %>% 
      ggplot()+
      geom_raster(aes(week, dist.trav, fill= difference_abs_mean))+
      scale_fill_viridis_c(name="absolute mean difference in SST [°C]", direction = -1)+
      geom_vline(xintercept = as.numeric(seq(as.POSIXct("2003/1/1", tz="GMT"), 
                                             as.POSIXct("2018/1/1", tz="GMT"), 
                                             "years")),
                 size=1)+
...
rm(comparison_hov_data,hov1,hov2, select_route)
```
