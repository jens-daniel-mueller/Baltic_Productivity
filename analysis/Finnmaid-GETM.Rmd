...
date_time_o <- ncvar_get(nc, "otime_east")
  ...
  for (i in seq(1,length(route),1)){
    ...
    if(route[i] == select_route) {
      ...
      temp <- bind_cols(value = value, lon = longitude_east, lat = latitude_east, date_time = date)
      temp$var <- var
...
for (m in seq(1, length(lat),1)){
 
  value <- array[m,]
  temp <- bind_cols(value = value, lon = lon, lat = lat)
      temp$var <- var
  
}
# mean per day, to then loop through coordinates`?
# array_2$t <- cut.POSIXt(array_2$t,breaks =  "days")
# array_mean <- array_2 %>% 
#      group_by(t) %>% 
#      summarise_all(list(~mean(.,na.rm = TRUE)))
...
SST_GETM_mean %>% 
  write_csv(here::here("data/_summarized_data_files/", file = "GETM_SST_whole_mean_nolatlimits.csv"))
