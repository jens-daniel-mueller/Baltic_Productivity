...
```{r timeseries_dygraph, fig.asp=0.6}
...
ts_pco2_xts <- xts(ts_mld_pco2$value_pCO2, order.by = ts_mld_pco2$date)
names(ts_pco2_xts) <- "pCO2 Finnmaid"
...
# add sd as ribbon here
ts_pco2_xts %>% 
  dygraph(group = "MLD") %>% 
  dyRangeSelector(dateWindow = c("2014-01-01", "2016-12-31")) %>% 
  dySeries("pCO2 Finnmaid") %>% 
  dyAxis("y", label = "") %>% 
  dyOptions(drawPoints = TRUE, pointSize = 1.5, connectSeparatedPoints=TRUE, strokeWidth=0.5,
            drawAxesAtZero=TRUE)
