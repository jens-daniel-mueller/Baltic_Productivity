...
ts_mld_pco2_full_hov_2 <- ts_mld_pco2_full_hov %>% 
  mutate(year = year(date),
         week = as.Date(cut(ts_mld_pco2_full$date, breaks="weeks")))

ts_mld_pco2_full_hov_2_naremove <- na.omit(ts_mld_pco2_full_hov_2)

ts_mld_pco2_full_hov_2_naremove %>%
  filter(year > 2002, year < 2019) %>% 
    ggplot()+
    geom_raster(aes(week, dist.trav, fill=value_pCO2_Finn))+
    #scale_fill_scico(palette = "vik", name="mean difference in SST [°C]")+
...
comparison_hov_pco2_mld <- ts_mld_pco2_full_hov_2 %>% 
  na.omit() %>% 
  mutate(difference = (value_pCO2_Finn - value_mld5))

comparison_hov_pco2_mld %>%
  filter(year > 2006, year < 2019) %>% 
    ggplot()+
    geom_raster(aes(week, dist.trav, fill= difference))+
    #scale_fill_scico(palette = "vik", name="mean difference in SST [°C]")+
    scale_fill_divergent(name="Mean weekly mld-pCO2  difference GETM-FM", na.value = "white",
                         mid = 'grey')+
    scale_x_date(expand = c(0,0))+
    scale_y_continuous(expand = c(0,0))+
    labs(y="Distance to Travemuende [km]")+
...
rm(ts_mld_pco2_full_hov_2_naremove, ts_mld_pco2_full_hov_2, ts_mld_pco2_full_hov, comparison_hov_pco2_mld)
```
# Medium Salinity 
