...
date_time_o <- ncvar_get(nc, "otime_east")
  ...
  for (i in seq(1,length(route),1)){
    ...
    if(route[i] == select_route) {
      slice <- array[i,]
      date_o <- date_time_o[i,]
      
      value <- slice
      date <- ymd("2000-01-01") + date_time[i]
      
      temp <- bind_cols(value = value, lon = longitude_east, lat = latitude_east, date_time = date_o)
      temp$var <- var
      temp$date <- date
    ...
    for (a in 1:544){
  timeseries$date_time_o[a] <- ymd("2000-01-01") + timeseries$date_time_o[a]
  }
...
timeseries$date_time <- as.POSIXct(timeseries$date) 
timeseries$date_time_o <- as.POSIXct(timeseries$date_time_o)
