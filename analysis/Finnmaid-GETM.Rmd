...
rm(comparison_hov_data, select_route)
```

# pCO2 readings and mixed layer depths
## Mixed layer depths
...
We want a datafile including the pCO2 values, as measured from the Finnmaid, as well as all five mixed layer depth parameters. For the pCO2 values, we want the corresponding minimum, maximum and standard deviation values per date. 

```{r merging_mld_pco2, eval= TRUE}
...
```{r timeseries pCO2 dygraphs}

ts_mld_pco2 <- read_csv(here::here("data/_merged_data_files/", file = "Finnmaid_GETM_pCO2_mld_latrestriction.csv"))
...
* Request 2003-2005 GETM data
* Request GETM wind data
* Include temperature 2-4 m in 2d GETM files
* Quantify diurnal SST variability in GETM 2d file, in order to estimate biases introduced by comparing daily mean values
* Why are there some Finnmaid observations missing with low distance to Travemuende?
