...
timeseries %>% 
  write_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_SST_whole_nolatlimits.csv"))

#rm(array, nc, slice, var, date_time_o)
```
...
rm(SST_GETM_whole, n, file, lon, lat, var, filesList_2d)
...
SST_Finn <- 
  read_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_SST_whole_nolatlimits.csv"))
...
comparison$difference <- abs(comparison$value_SST_Finn-comparison$value_SST_GETM)
...
hov1 <-
      ...
      scale_fill_viridis_c(name="mean difference", direction = -1)+
