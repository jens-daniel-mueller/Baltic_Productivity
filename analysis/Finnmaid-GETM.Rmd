...
SST_GETM_whole %>% 
  write_csv(here::here("data/_summarized_data_files/", file = "GETM_SST_whole_nolatlimits.csv"))
...
SST_GETM <- 
  read_csv(here::here("data/_summarized_data_files/", file = "GETM_SST_whole_nolatlimits.csv"))
SST_Finn <- 
...
SST_combined_GETM_Finn <-
  inner_join(SST_Finn, SST_GETM, by = "date")
  
#comparison$week <- as.Date(cut(comparison$date_time, breaks="weeks"))


#raster lat 

#comparison$dist.trav <- distGeo(cbind(sub.df.hov$Lon, sub.df.hov$Lat), trav) / 1e3
