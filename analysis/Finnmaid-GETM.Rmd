...
This following analysis are performed for observation along the Finnmaid route `r select_route`.
...
rm(SST_GETM_whole, n, file, time_units, var)
...
array[array == fillvalue$value] <- NA
    ...
    for (i in seq(1,length(t),1)){
      ...
      if (exists("SST_GETM_whole_3d")) {
        SST_GETM_whole_3d <- bind_rows(SST_GETM_whole_3d, SST_GETM_3d)
...
rm(SST_GETM_whole_3d, n, file, filesList_3d, filesList_2d, d1_deep,d1_shallow,var_3d,lat,i)
...
comparison %>% 
  write_csv(here::here("data/_merged_data_files/", file = "Finnmaid_GETM_mean_SST_latrestriction.csv"))

rm(comparison, SST_GETM_mean,SST_GETM_whole,SST_GETM_whole_3d,timeseries)
...
ts <- read_csv(here::here("data/_merged_data_files/", file = "Finnmaid_GETM_mean_SST_latrestriction.csv"))
...
#Rerun data preparation without lat and lon limits, save lon var as well
...
timeseries %>% 
  write_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_SST_whole_nolatlimits_corvector.csv"))
...
#use corvector (1:544)
# this ends with corvector being NA, check tomorrow
SST_GETM_corvector <- array %>%
...
if (exists("SST_GETM_whole")) {SST_GETM_whole <- bind_rows(SST_GETM_whole, SST_GETM_corvector)}
        else {SST_GETM_whole <- SST_GETM_corvector}

nc_close(nc)
rm(array, nc, t, time_units, SST_GETM_corvector)

print(n)
}

SST_GETM_corvector_whole <- SST_GETM_whole
# using tidyverse write_csv
# write only one data file
# use relative path with here package

SST_GETM_corvector_whole %>% 
  write_csv(here::here("data/_summarized_data_files/", file = "GETM_SST_whole_nolatlimits_corvector.csv"))


rm(SST_GETM_whole, n, file, lon, lat, var, filesList_2d, SST_GETM_corvector_whole, corvector)
...
SST_GETM <- 
  read_csv(here::here("data/_summarized_data_files/", file = "GETM_SST_whole_nolatlimits_corvector.csv"))
SST_Finn <- 
  read_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_SST_whole_nolatlimits_corvector.csv"))
...
SST_combined_GETM_Finn <-full_join(SST_Finn, SST_GETM, by = c("date", "corvector"))
SST_combined_GETM_Finn_dropNA <- drop_na(SST_combined_GETM_Finn)

SST_combined_GETM_Finn %>% 
  write_csv(here::here("data/_merged_data_files/", file = "SST_combined_GETM_Finn_corvector.csv"))
SST_combined_GETM_Finn_dropNA %>% 
  write_csv(here::here("data/_merged_data_files/", file = "SST_combined_GETM_Finn_corvector_dropNA.csv"))
  

comparison <- SST_combined_GETM_Finn

comparison$week <- as.Date(cut(comparison$date, breaks="weeks"))
comparison$dist.trav <- comparison$corvector*2
comparison$difference <- comparison$value_SST_Finn-comparison$value_SST_GETM


# final data for hovmoeller plots
comparison_hov_data <- comparison %>%
      select(value_SST_Finn, value_SST_GETM, difference,week, dist.trav) %>% 
      dplyr::group_by(dist.trav, week) %>% 
      summarise_all(list(mean=~mean(.,na.rm=TRUE))) %>% 
      as.data.frame()
#comparison_hov_data$week <- as.POSIXct(comparison_hov_data$week)

comparison_hov_data %>% 
  write_csv(here::here("data/_merged_data_files/", file = "SST_hovmoeller_GETM_Finn.csv"))

rm(comparison,comparison_hov_data,SST_combined_GETM_Finn,SST_combined_GETM_Finn_dropNA,SST_Finn, SST_GETM)
```
...
# Hovmoeller Plot, difference SST Finn - SST GETM
comparison_hov_data <-
    read_csv(here::here("data/_merged_data_files/", file = "SST_hovmoeller_GETM_Finn.csv"))

comparison_hov_data <- comparison_hov_data %>% 
...
mld_GETM_whole %>% 
  write_csv(here::here("data/_summarized_data_files/", file = paste("GETM_", var, "_whole_corvector.csv", sep = "")))
...
timeseries <- timeseries %>% 
  select(-c(date))
timeseries %>% 
  write_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_pCO2_whole.csv"))
...
## change pco2 reading to without lat restrictions but write lat & lon or coordinat vector as well
## combine all, in plot filter for the lat restrictions, that we way we can save one round of data prep
ts_pCO2 <- read_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_pCO2_whole.csv"))
ts_mld_age_1 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_1_whole_corvector.csv"))
ts_mld_age_3 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_3_whole_corvector.csv"))
ts_mld_age_5 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_5_whole_corvector.csv"))
ts_mld_rho <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_rho_whole_corvector.csv"))
ts_mld_tke <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_tke_whole_corvector.csv"))
...
#ts_mld_pco2_inner <- inner_join(ts_pCO2, ts_mld, by = "date")

ts_mld_pco2_full %>% 
write_csv(here::here("data/_merged_data_files/", file = "Finnmaid_GETM_pCO2_mld_latrestrictions.csv"))
...
ts_mld_pco2 <- read_csv(here::here("data/_merged_data_files/", file = "Finnmaid_GETM_pCO2_mld_latrestrictions.csv"))
...
timeseries %>% 
  write_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_pCO2_whole_nolatlimits_corvector.csv"))
...
## change pco2 reading to without lat restrictions but write lat & lon or coordinat vector as well
## combine all, in plot filter for the lat restrictions, that we way we can save one round of data prep
ts_pCO2 <- read_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_pCO2_whole_nolatlimits_corvector.csv"))
ts_mld_age_1 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_1_whole_corvector.csv"))
ts_mld_age_3 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_3_whole_corvector.csv"))
ts_mld_age_5 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_5_whole_corvector.csv"))
ts_mld_rho <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_rho_whole_corvector.csv"))
...
ts_mld_pco2_full %>% 
write_csv(here::here("data/_merged_data_files/", file = "Finnmaid_GETM_pCO2_mld_nolatrestrictions.csv"))
...
ts_mld_pco2_full <- 
  read_csv(here::here("data/_merged_data_files/", file = "Finnmaid_GETM_pCO2_mld_nolatrestrictions.csv"), guess_max = 105537)
...
ts_mld_pco2_full_hov %>% 
  ...
  ggplot()+
    geom_raster(aes(date, dist.trav, fill=value_mld1))+
      scale_fill_viridis_c(name="GETM MLD Age 1 [m]", option = "B")+
      scale_x_date(expand = c(0,0))+
      scale_y_continuous(expand = c(0,0))+
      labs(y="Distance to Travemuende [km]")+
      theme_bw()+
...
ts_mld_pco2_full_hov %>% 
  ...
  ggplot()+
    geom_raster(aes(date, dist.trav, fill=value_mld3))+
      scale_fill_viridis_c(name="GETM MLD Age 3 [m]", option = "B")+
      scale_x_date(expand = c(0,0))+
      scale_y_continuous(expand = c(0,0))+
      labs(y="Distance to Travemuende [km]")+
      theme_bw()+
...
ts_mld_pco2_full_hov %>% 
  ...
  ggplot()+
    geom_raster(aes(date, dist.trav, fill=value_mld5))+
      scale_fill_viridis_c(name="GETM MLD Age 5 [m]", option = "B")+
      scale_x_date(expand = c(0,0))+
      scale_y_continuous(expand = c(0,0))+
      labs(y="Distance to Travemuende [km]")+
      theme_bw()+
...
ts_mld_pco2_full_hov %>% 
  ...
  ggplot()+
    geom_raster(aes(date, dist.trav, fill=value_mldrho))+
      scale_fill_viridis_c(name="GETM MLD Rho [m]", option = "B")+
      scale_x_date(expand = c(0,0))+
      scale_y_continuous(expand = c(0,0))+
      labs(y="Distance to Travemuende [km]")+
      theme_bw()+
...
#mld tke
# there is a problem in the original data, or nc import for this variable, there are negative values and values up to ~3000, 
# that for the second filter (filter(value_mldtke >0, value_mldtke < 300)) was added, to circumvent the issue
ts_mld_pco2_full_hov %>% 
  filter(year > 2006, year < 2019) %>%
  filter(value_mldtke >0, value_mldtke < 300) %>% 
  ggplot()+
    geom_raster(aes(date, dist.trav, fill=value_mldtke))+
      scale_fill_viridis_c(name="GETM MLD Tke [m]", option = "B")+
      scale_x_date(expand = c(0,0))+
      scale_y_continuous(expand = c(0,0))+
      labs(y="Distance to Travemuende [km]")+
      theme_bw()+
...
# Is there a possibility to show years in rows and different mld parameters as columns? Would that be more informative?
```
...
ts_pco2_full <-
read_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_pCO2_whole_nolatlimits_corvector.csv"))


#ts_mld_pco2_full_hov <- ts_mld_pco2_full %>% 
#  mutate(date = ymd(date), 
#          week = as.Date(cut(ts_mld_pco2_full$date, breaks="weeks")),
#         dist.trav = corvector*2)

#comparison_hov_pco2_mld <- ts_mld_pco2_full_hov %>% 
#  mutate(relation = (value_pCO2_Finn/value_mld5)) %>% 
#  select(value_pCO2_Finn, value_mld5, relation, week, dist.trav) %>% 
#  dplyr::group_by(dist.trav, week) %>% 
#  summarise_all(list(mean=~mean(., na.rm = TRUE))) %>% 
#  as.data.frame %>% 
#  mutate(year = year(week)) #%>% 
...
ts_pco2_full_hov <- ts_pco2_full %>% 
  mutate(date = ymd(date),
         week = as.Date(cut(ts_pco2_full$date, breaks="weeks")),
         dist.trav = corvector*2)
...
ts_pco2_full_hov %>%
  filter(year > 2006, year < 2019) %>% 
    ...
    #scale_fill_scico(palette = "vik", name="mean difference in SST [°C]")+
    scale_fill_viridis_c(name="Finnmaid pCO2 [ppm]", na.value = "white", option = "B")+
    scale_x_date(expand = c(0,0))+
    scale_y_continuous(expand = c(0,0))+
    labs(y="Distance to Travemuende [km]")+
    theme_bw()+
...
comparison_hov_pco2_mld %>%
  ...
  dplyr::filter(relation_mean > 0 , relation_mean < 800) %>% 
    ggplot()+
    geom_raster(aes(week, dist.trav, fill= relation_mean))+
    #scale_fill_scico(palette = "vik", name="mean difference in SST [°C]")+
    scale_fill_divergent(name="Mean weekly pCO2/mld5  relation GETM-FM", na.value = "white",
                         mid = 'grey')+
    scale_x_date(expand = c(0,0))+
    scale_y_continuous(expand = c(0,0))+
    labs(y="Distance to Travemuende [km]")+
...
rm(ts_mld_pco2_full_hov_2_naremove, ts_mld_pco2_full_hov, ts_mld_pco2_full_hov, comparison_hov_pco2_mld)
```
# Medium Salinity 
...
finnmaid_sss_corvector <- timeseries %>% 
  mutate(sss = value,
         year = year(date),
         month = month(date))
  ...
  for (cor in 1:544){
    ...
    temp <- finnmaid_sss_corvector %>% 
      filter(corvector == cor) %>% 
      select(month, corvector, sss) %>% 
      group_by(month, corvector) %>% 
      summarise_all(mean, na.rm = TRUE)
    
    if (exists("monthly_salinity", inherits = FALSE)){
        monthly_salinity <- bind_rows(monthly_salinity, temp)
      } else{monthly_salinity <- temp}
    
  }
  

monthly_salinity %>% 
write_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_monthly_Salinity_perTransect_short.csv"))


rm(monthly_salinity, temp, finnmaid_sss_corvector, timeseries)
```

```{r scatterplot_salinity}

monthly_salinity <- read_csv(here::here("data/_summarized_data_files/", file = "Finnmaid_monthly_Salinity_perTransect.csv"))

monthly_salinity$month <- as.factor(monthly_salinity$month)

monthly_salinity %>% 
ggplot(aes(corvector,sss, color = month))+
  geom_path()+
...
* Check why first 6 GETM nc files do not open  -> revised file expected in Feb 2020
* Request 2003-2005 GETM data -> expected in Feb 2020
* Request GETM wind data -> expected in Feb 2020
* Quantify diurnal SST variability in GETM 2d file, in order to estimate biases introduced by comparing daily mean values
* Why are there some Finnmaid observations missing with low distance to Travemuende? -> guess ferrybox is switched of in harbor window
