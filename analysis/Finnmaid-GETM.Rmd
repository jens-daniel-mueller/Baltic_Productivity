...
var <- var_all[a]
  ...
  mld_GETM <- array %>%
    ...
    gather("lat", "value", 1:length(lat)) %>%
    mutate(lat = as.numeric(lat)) %>%
    #comment the following line to get dataset without lat restrictions
    #filter(lat > low_lat, lat<high_lat) %>%
    group_by(date_time,lat) %>%
    summarise_all("mean")
   


  if (exists("mld_GETM_whole")) {mld_GETM_whole <- bind_rows(mld_GETM_whole, mld_GETM)
  }else {mld_GETM_whole <- mld_GETM}

  nc_close(nc)
  rm(array, nc, t, lat, mld_GETM)
  print(n) # to see working progress
}

print(paste("GETM_", var, "_whole.csv", sep = "")) # to see working progress
mld_GETM_whole %>% 
  write_csv(here::here("data/_summarized_data_files/", file = paste("GETM_", var, "_whole.csv", sep = "")))
...
ts_mld_age_1 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_1_whole.csv"))
ts_mld_age_3 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_3_whole.csv"))
ts_mld_age_5 <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_age_5_whole.csv"))
ts_mld_rho <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_rho_whole.csv"))
ts_mld_tke <- read_csv(here::here("data/_summarized_data_files/", file = "GETM_mld_tke_whole.csv"))


ts_pCO2 <- ts_pCO2 %>% 
  transmute( value_pCO2 = value, sd_pCO2 = sd, min_pCO2 = min, max_pCO2 = max, date = date_time)

# try again with lat restrictions in getm? for now this is a different summarisation functionality than
# then mean will work like we want, now lat gets averaged which doesn't make any sense
ts_mld_age_1 <- ts_mld_age_1 %>%
  ...
  summarise_all(list(mean=~mean(.,na.rm=TRUE))) %>%
  ungroup() %>%
  transmute(value_mld1 = mean, date = date, lat = lat)
...
ts_pCO2_mld <- read_csv(here::here("data/_merged_data_files/", file = "Finnmaid_GETM_pCO2_mld_latrestriction.csv"))
